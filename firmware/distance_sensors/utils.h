...
// If you want to use 32 bits at a time, then only BANK0 and BANK2 should be used for BANK
// For 64 bits at a time, only BANK0 should be used for BANK
#define SET_BITS(BITFIELD, BITS, BANK)          (BITFIELD |= ((uint64_t)(BITS) << BANK))
#define CLEAR_BITS(BITFIELD, BITS, BANK)        (BITFIELD &= ~((uint64_t)(BITS) << BANK))
#define CHECK_BITS(BITFIELD, BITS, BANK)        ((BITFIELD >> BANK) & (uint64_t)(BITS))                       // Checks if any of the given bits are set
#define STRICT_CHECK_BITS(BITFIELD, BITS, BANK) (((BITFIELD >> BANK) & (uint64_t)(BITS)) == (uint64_t)(BITS)) // Checks if all of the given bits are set
